	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double learningRate = alpha(Ntable.get(prev_stateaction));  	// α
				double reward = previous_reward;                               	// R
				double discountFactor = GAMMA_DISCOUNT_FACTOR;                 	// γ
				double maxNextStateActionValue = getMaxActionQValue(new_state); // max(Q(s', a'))
				double currentQValue = Qtable.get(prev_stateaction);         	// Q(s, a)

				// Q-learning update equation
				double updatedQValue = currentQValue + learningRate * (reward + discountFactor * maxNextStateActionValue - currentQValue);

				Qtable.put(prev_stateaction, updatedQValue);  // Update Q(s, a)
				...
				if (print_counter % 10 == 0) {
					System.out.println("ITERATION: " + iteration + " SENSORS: a=" + df.format(angle.getValue()) + " vx=" + df.format(vx.getValue()) +  " vy=" + df.format(vy.getValue()) + " P_STATE: " + previous_state + " P_ACTION: " + previous_action + " P_REWARD: " + df.format(previous_reward) + " P_QVAL: " + df.format(Qtable.get(prev_stateaction)) + " Tested: " + Ntable.get(prev_stateaction) + " times.");
				}
